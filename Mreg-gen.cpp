...
class Mreg_gen : public Mreg<T>{
	...
	void append(const char* expression, const char* nickname){
	}
...
int main(int argc, char * argv[]){
	...
	Mreg_gen<uintptr_t> r = Mreg_gen<uintptr_t>();
	...
	auto store_data_start = Clock::now();
	r.store(file);
	auto store_data_end = Clock::now();
