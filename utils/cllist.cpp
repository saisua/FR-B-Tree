...
class C_linked_list {
    private:
        ...
        const unsigned long _CACHE_SIZE = sysconf (_SC_LEVEL1_DCACHE_LINESIZE);
    ...
    public:
        ...
        inline T* at(uint pos){
            if(!this->_modified_coll_size){
                return this->_array_starts[pos/this->_collection_size]+(pos%this->_collection_size);
            }
        ...
        inline T* at_node(uint pos){
            return this->_pointers[pos];
        }
        ...
        inline T* append_node(uint positions=NULL, bool add_size=false){
            if(!positions)
                if(this->_node_size)
        ...
        inline void set_current_collection(T* array, uint index, T* next_array, uint next_index){
            ++this->collections;
        ...
        void set_node_size(const uint nsize, uint nodes_per_coll=10, uint at_least_nodes=20 ) {
            ...
            uint mod_cache_size = this->_CACHE_SIZE % nsize;
            ...
            if(! this->_flags & COLL_S) {
                ...
                if(this->_node_size > this->_CACHE_SIZE)
                    this->_collection_size = this->_node_size * this->_CACHE_SIZE;
                else
                    if(this->_CACHE_SIZE * 2 >= this->_node_size * nodes_per_coll)
                        this->_collection_size = this->_CACHE_SIZE * 2;
                    else{
                        uint mod_cache_coll;
                        ...
                        if(mod_cache_coll = (this->_CACHE_SIZE % this->_collection_size))
                            this->_collection_size += (this->_node_size * 
                                    (((this->_CACHE_SIZE - (mod_cache_coll)) / this->_node_size)+1));
                    }
        ...
        void set_collection_size(const uint csize) {
            ...
            // Tl;dr: add the node size to the collection size until it
            // surpasses the next cache line size multiple.
            if(this->mod_cache_coll = this->_CACHE_SIZE % this->_collection_size)
                this->_collection_size = csize + (this->_node_size * 
                        (((this->_CACHE_SIZE - this->mod_cache_coll) / this->_node_size)+1));
